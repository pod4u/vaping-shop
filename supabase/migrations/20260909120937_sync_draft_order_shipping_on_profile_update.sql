...
declare
  v_address_id uuid;
  v_draft_orders_updated integer := 0;
begin
  ...
  if v_address_id is null then
    insert into public.customer_addresses (
      customer_id, recipient_name, phone, address, province, postal_code, is_default
    ) values (
      p_customer_id, btrim(p_recipient_name), btrim(p_phone), btrim(p_address),
      btrim(p_province), nullif(btrim(p_postal_code), ''), true
  ...
  set source_address_id = v_address_id,
      shipping_name = btrim(p_recipient_name),
      shipping_phone = btrim(p_phone),
      shipping_address = btrim(p_address),
      shipping_province = btrim(p_province),
      shipping_postal_code = nullif(btrim(p_postal_code), '')
  where customer_id = p_customer_id
    and status = 'draft';
  get diagnostics v_draft_orders_updated = row_count;

  return jsonb_build_object(
    'customer_id', p_customer_id,
    'address_id', v_address_id,
    'draft_orders_updated', v_draft_orders_updated,
    'updated', true
  );
end;
$$;
...
comment on function public.update_member_profile(integer, uuid, text, text, text, text, text, text) is
  'Atomically updates a verified member profile, default shipping address, and all unconfirmed draft order shipping snapshots.';
